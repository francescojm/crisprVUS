...
tissues<-setdiff(tissues,c('Other Solid Carcinomas','Other Solid Cancers','Other Sarcomas', "Non-Cancerous", "Other Blood Cancers"))
...
hits_nop<-c()
for(ctiss in tissues){
  hits_nop<-c(hits_nop, unique(results[[ctiss]]$GENE[results[[ctiss]]$rank_ratio<1.6 & results[[ctiss]]$medFitEff< -.5]))
}

hits_nop<-unique(hits_nop)
save(hits_nop, file="hits_nop.RData")
...
fun_tiss<-function(x){
  ...
  for(i in 1:1000){
    print(i)
    RESTOT_rand[[i]] <- fun_iter(iter=i, ctiss=tt)
  }
  save(RESTOT_rand,file=paste(resultPath,'/',tt,'_results_rand.RData',sep=''))
}

fun_iter<-function(iter, ctiss){
  res<-source(paste(pathscript,'/_VUS2024build_01_mut_CRISPR_rand.R', sep=""), verbose=F, local=T)
  return(res)
}

numCores <- detectCores()
n_tiss<-c(1:length(tissues))
mclapply(n_tiss, fun_tiss, mc.cores = numCores)
