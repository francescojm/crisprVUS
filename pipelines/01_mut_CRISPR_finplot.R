#same as 01_mut_CRISPR, changing the plots' aesthetics for the manuscript.
  ...
  if (sum(hits)>0){
    ...
    if (display){
      ...
      bg[ps_cl]<-'red'
        ...
        if(length(save)>0){
          pdf(paste(save,gene,'_',paste(gsub("\\?|\\*|!", "", vs),collapse='AND'),'.pdf',sep=''),length(bg)*0.2+0.5,4)
        }
        
        
        plot(FCp,col=bg,pch=pcpatt,xlab='cell lines',
             ylab=paste(gene,'scaled fitness effect'),
             main=paste('Rank ratio =',format(sum(hits)/((n*(n+1))/2),digits=3)))
        abline(h= -1,col='gray')
...
dir.create(paste(resultPath,'/_CRISPR_plots_fin/',sep=''))
...
RESTOT<-multipleGeneClasTests(ess_scores = ts_depFC,
                              ...
                              genes = genesToTest,
                              path = paste(resultPath,'/_CRISPR_plots_fin/',sep=''))
