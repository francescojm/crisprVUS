...
#counts the number of mutations per gene
...
home<-"\\\\storage.fht.org/home/VUS/VUS"
...
hits<-c()
for(ctiss in tissues){
  hits<-c(hits, unique(results[[ctiss]]$GENE[results[[ctiss]]$rank_ratio<1.6 & results[[ctiss]]$medFitEff< -.5 ]))
}

hits<-unique(hits)
save(hits, file="hits_nop.RData")
