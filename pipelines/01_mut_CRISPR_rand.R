#same code as 01_mut_CRISPR, but reshuffling cell lines in essentiality data
...
positives<-function(cl_var,gene,variants){
  return(sort(unique(cl_var$model_name[cl_var$gene_symbol_2019==gene & is.element(cl_var$protein_mutation,variants)])))
}
clasTest<-function(ess_scores,bess_scores,cl_var,gene,vs,vs_cds,display=F,save=NULL){
...
Optimal_clasTest<-function(ess_scores,bess_scores,cl_var,gene,vs,vs_cds,display=F,save=NULL){
  ...
  if(res$rank_ratio<1.6 & res$medFitEff< -.5){
    ...
    clasTest(ess_scores = ess_scores,
             ...
             cl_var = cl_var,
             gene = gene,
             vs=curr_var,vs_cds = curr_var_cds,display = F,save=save)
  }
...
multipleGeneClasTests<-function(ess_scores,bess_scores,cl_var,genes,path){
  ...
  genes<-intersect(genes,cl_var$gene_symbol_2019)
  ngenes<-length(genes)
  rand_ind<-sample(1:ncol(ess_scores), ncol(ess_scores), replace = F)
  cl<-colnames(ess_scores)
  ess_scores<-ess_scores[,rand_ind]
  colnames(ess_scores)<-cl
  bess_scores<-bess_scores[,rand_ind]
  colnames(bess_scores)<-cl
...
#run randomizations only on DAMs identified in 01_mut_CRISPR.R
load("results/20220208/hits_nop.RData")
genesToTest<-hits
...
if(!dir.exists(paste(resultPath,'/_CRISPR_plots_rand/',sep=''))){
  dir.create(paste(resultPath,'/_CRISPR_plots_rand/',sep=''))  
}

dir.create(paste(resultPath,'/_CRISPR_plots_rand/',ctiss,sep=''))

RESTOT<-multipleGeneClasTests(ess_scores = ts_depFC,
                              bess_scores = ts_bdep,
                              cl_var = ts_cl_variants,
                              genes = genesToTest,
                              path = paste(resultPath,'/_CRISPR_plots_rand/',ctiss,'/',sep=''))
