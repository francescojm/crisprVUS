...
load("../../data/HT29R.reproducible_GeneGuides.RData")
reproducible_genes<-unique(sub("\\_.*", "", HT29R.reproducible_GeneGuides))
scaled_depFC<-scaled_depFC[which(rownames(scaled_depFC) %in% reproducible_genes),]
expression<-scaled_depFC
...
save(misclass_result_all, file="misclass_ess_rank.RData")
