...
#   if(length(which(res$rank_ratio<RR_th))==0)
    if(length(which(res$rank_ratio<RR_th & res$medFitEff< -.5))==0){
  ...
  if(res$rank_ratio<RR_th & res$medFitEff< -.5){
    ...
    clasTest(ess_scores = ess_scores,
             ...
             cl_var = cl_var,
             gene = gene,
             vs=curr_var,vs_cds = curr_var_cds,display = display,save=save)
  }
...
multipleGeneClasTests<-function(ess_scores,bess_scores,cl_var,genes,path,display=TRUE){
  ...
  for (i in 1:ngenes){
    ...
    curRES<-Optimal_clasTest(ess_scores = ess_scores,
                             ...
                             vs = vs,
                             vs_cds = vs_cds,
                             save = path,display = display)
    
    if(curRES$rank_ratio<RR_th & curRES$medFitEff< -.5){
