...
library(openxlsx)
proteome<-read.xlsx("../../data/Proteome_TableS2.xlsx",2)
rownames(proteome)<-unlist(strsplit(proteome[,1], ";"))[seq(1,nrow(proteome)*2,2)]
proteome<-proteome[,-1]
colnames(proteome)<-gsub("_HUMAN", "", unlist(strsplit(as.character(proteome[1,]), ";"))[seq(2,ncol(proteome)*2,2)])
proteome<-proteome[-1,]
proteome_mat <- matrix(as.numeric(unlist(proteome)),    # Convert to numeric matrix
                       ncol = ncol(proteome))
proteome_mat<-t(proteome_mat)
colnames(proteome_mat)<-rownames(proteome)
rownames(proteome_mat)<-colnames(proteome)
...
extracor<-cor(expression[,all_lines_sel], use="pairwise.complete.obs")
...
for(ctiss in tissues){
  ...
  for(othertype in type){
    ...
    cor_int<-cor(expression[,all_lines_ctiss], use="pairwise.complete.obs")
...
save(misclass_result_all, file="misclass_prot_rank.RData")
